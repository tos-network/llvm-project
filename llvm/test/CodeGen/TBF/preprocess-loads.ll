; RUN: llc -O2 -march=tbf -filetype=asm < %s | FileCheck --check-prefix=CHECK %s
; RUN: llc -O2 -march=tbf -filetype=asm -mattr=+mem-encoding < %s | FileCheck --check-prefix=CHECK %s
