; RUN: llc -march=tbf -mattr=+store-imm -tbf-enable-btf-emission < %s | FileCheck %s
; RUN: llc -march=tbf -mattr=+store-imm,+mem-encoding -tbf-enable-btf-emission < %s | FileCheck %s
...
;
;   clang -g -O2 -S -emit-llvm --target=tbf test.c
