; RUN: llc < %s -march=tbf -mattr=+store-imm -show-mc-encoding | FileCheck %s
; RUN: llc < %s -march=tbf -mattr=+store-imm,+alu32 -show-mc-encoding | FileCheck %s
; RUN: llc < %s -march=tbf -mattr=+store-imm,+mem-encoding -show-mc-encoding | FileCheck --check-prefix=CHECK-V2 %s
; RUN: llc -march=tbf -mcpu=v3 -filetype=obj -o - %s | llvm-objdump -d --no-print-imm-hex - | FileCheck --check-prefix=CHECK-DUMP %s
...
; TBF allows misaligned memory accesses
