; RUN: llc -O2 -march=tbf -mcpu=v1 < %s | FileCheck %s
; RUN: llc -O2 -mtriple=tbpfv1-tos-tos < %s | FileCheck %s
; RUN: llc -O2 -march=tbf -mcpu=v1 -mattr=+mem-encoding < %s | FileCheck %s
; RUN: llc -O3 -march=tbf -mcpu=v3 < %s | FileCheck --check-prefix=CHECK-V3 %s
