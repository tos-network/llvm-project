; RUN: opt -O2 -mtriple=tbf %s | llvm-dis > %t1
; RUN: llc -tbf-enable-btf-emission -filetype=asm -o - %t1 | FileCheck -check-prefixes=CHECK %s
; RUN: llc -tbf-enable-btf-emission -mattr=+alu32 -filetype=asm -o - %t1 | FileCheck -check-prefixes=CHECK %s
;
; Source code:
