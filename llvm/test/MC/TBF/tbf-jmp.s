# RUN: llvm-mc %s -triple=tbf-tos-tos --mcpu=v3 --show-encoding \
# RUN:     | FileCheck %s --check-prefix=CHECK-ASM-NEW
# RUN: llvm-mc %s -triple=tbpfv3-tos-tos --show-encoding \
# RUN:     | FileCheck %s --check-prefix=CHECK-ASM-NEW
# RUN: llvm-mc %s -triple=tbf-tos-tos --show-encoding \
# RUN:     | FileCheck %s --check-prefix=CHECK-ASM-OLD
# RUN: llvm-mc %s -triple=tbf-tos-tos --mcpu=v3 -filetype=obj \
# RUN:     | llvm-objdump -d -r - \
# RUN:     | FileCheck --check-prefix=CHECK-OBJ-NEW %s
# RUN: llvm-mc %s -triple=tbf-tos-tos -filetype=obj \
# RUN:     | llvm-objdump -d -r - \
