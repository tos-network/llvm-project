# RUN: llvm-mc %s -triple=tbf-tos-tos -mattr=+mem-encoding,+pqr-instr --show-encoding \
# RUN:     | FileCheck %s --check-prefix=CHECK-ASM-NEW
# RUN: llvm-mc %s -triple=tbf-tos-tos -mcpu=tbfv2 -mattr=+mem-encoding,+pqr-instr -filetype=obj \
# RUN:     | llvm-objdump --mattr=+mem-encoding,+pqr-instr -d -r - \
# RUN:     | FileCheck --check-prefix=CHECK-OBJ-NEW %s
# RUN: llvm-mc %s -triple=tbf-tos-tos -mattr=+alu32,+mem-encoding,+pqr-instr --show-encoding \
# RUN:     | FileCheck %s --check-prefix=CHECK32-ASM-NEW
# RUN: llvm-mc %s -triple=tbf-tos-tos -mcpu=tbfv2 -mattr=+alu32,+mem-encoding,+pqr-instr -filetype=obj \
