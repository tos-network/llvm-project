...
SDValue TBFTargetLowering::LowerFormalArguments(
  ...
  if (!Subtarget->getHasDynamicFrames() && Ins.size() > MaxArgs) {
    ...
    CCInfo.AnalyzeFormalArguments(Ins,
                                  getHasAlu32() ? CC_TBF32_X : CC_TBF64_X);
  } else {
    // Pass args 1-5 via registers, remaining args via stack, if any.
    CCInfo.AnalyzeFormalArguments(Ins, getHasAlu32() ? CC_TBF32 : CC_TBF64);
  }
...
SDValue TBFTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  if (Outs.size() > MaxArgs) {
    if (Subtarget->getHasDynamicFrames()) {
      ...
      CCInfo.AnalyzeCallOperands(Outs, getHasAlu32() ? CC_TBF32 : CC_TBF64);
    } else {
      // Pass args 1-4 via registers, remaining args via stack, referenced via
      // TBF::R5
      CCInfo.AnalyzeCallOperands(Outs, getHasAlu32() ? CC_TBF32_X : CC_TBF64_X);
    }
  } else {
...
bool TBFTargetLowering::CanLowerReturn(
  ...
  return CCInfo.CheckReturn(Outs, getHasAlu32() ? RetCC_TBF32 : RetCC_TBF64);
}
...
TBFTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  CCInfo.AnalyzeReturn(Outs, getHasAlu32() ? RetCC_TBF32 : RetCC_TBF64);
...
SDValue TBFTargetLowering::LowerCallResult(
  ...
  CCInfo.AnalyzeCallResult(Ins, getHasAlu32() ? RetCC_TBF32 : RetCC_TBF64);
...
#define SET_NEWCC(X, Y)                                                        \
  ...
  // TBF at the moment only has 64-bit comparison. Any 32-bit comparison needs
