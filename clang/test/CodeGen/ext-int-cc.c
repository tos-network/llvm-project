...
// RUN: %clang_cc1 -no-enable-noundef-analysis -triple tbf -O3 -disable-llvm-passes -emit-llvm -o - %s | FileCheck %s --check-prefixes=TBF
...
// TBF: define{{.*}} void @ParamPassing(i128 %{{.+}}, i64 %{{.+}})
...
// TBF: define{{.*}} i127 @ReturnPassing3(
